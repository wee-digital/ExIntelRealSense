...
namespace rs2 {
        ...
        */
        recording_context(const std::string &filename,
            ...
            _context = std::shared_ptr<rs2_context>(
                    rs2_create_recording_context(RS2_API_VERSION, filename.c_str(), section.c_str(),
                                                 mode, &e),
                    rs2_delete_context);
            error::handle(e);
        }
    ...
    public:
        ...
        */
        mock_context(const std::string &filename,
            ...
            rs2_error *e = nullptr;
            _context = std::shared_ptr<rs2_context>(
                    rs2_create_mock_context_versioned(RS2_API_VERSION, filename.c_str(),
                                                      section.c_str(), min_api_version.c_str(), &e),
                    rs2_delete_context);
            error::handle(e);
        }
    ...
    public:
        explicit software_device_destruction_callback(T on_destruction) : on_destruction_function(
                on_destruction) {}
    ...
    public:
        ...
        */
        stream_profile add_video_stream(rs2_video_stream video_stream, bool is_default = false) {
            ...
            auto profile = rs2_software_sensor_add_video_stream_ex(_sensor.get(), video_stream,
                                                                   is_default, &e);
            error::handle(e);
        ...
        */
        stream_profile add_motion_stream(rs2_motion_stream motion_stream, bool is_default = false) {
            ...
            auto profile = rs2_software_sensor_add_motion_stream_ex(_sensor.get(), motion_stream,
                                                                    is_default, &e);
            error::handle(e);
        ...
        */
        stream_profile add_pose_stream(rs2_pose_stream pose_stream, bool is_default = false) {
            ...
            auto profile = rs2_software_sensor_add_pose_stream_ex(_sensor.get(), pose_stream,
                                                                  is_default, &e);
            error::handle(e);
    ...
    private:
        ...
        software_sensor(std::shared_ptr <rs2_sensor> s)
                : rs2::sensor(s) {
            rs2_error *e = nullptr;
            if (rs2_is_sensor_extendable_to(_sensor.get(), RS2_EXTENSION_SOFTWARE_SENSOR, &e) ==
                0 && !e) {
                _sensor = nullptr;
            }
            rs2::error::handle(e);
        ...
        */
        ...
        void set_destruction_callback(T callback) const {
            ...
            rs2_software_device_set_destruction_callback_cpp(_dev.get(),
                                                             new software_device_destruction_callback<T>(
                                                                     std::move(callback)), &e);
            error::handle(e);
        }
