...
namespace rs400 {
    ...
    public:
        advanced_mode(rs2::device d)
            ...
            rs2_error *e = nullptr;
            if (_dev &&
                rs2_is_device_extendable_to(_dev.get(), RS2_EXTENSION_ADVANCED_MODE, &e) == 0 &&
                !e) {
                _dev = nullptr;
            }
            rs2::error::handle(e);
...
inline std::ostream &operator<<(std::ostream &o, rs2_rs400_visual_preset preset) {
    return o << rs2_rs400_visual_preset_to_string(preset);
}
